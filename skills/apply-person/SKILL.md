...
2. Read the System Index, the Ground Truth Database, the current CV list, the selected-lane tracker tab, and any semantically equivalent approved reusable answers.
...
- Select the most relevant current CV. Verify its filename and content before upload; do not choose an old PDF merely because it is convenient.
...
- Before submission, capture a literal pre-submit snapshot containing every visible question and exact response, selections, intentional blanks, uploaded filenames and links, consents, and any fee or commitment. Exclude passwords, one-time codes, CAPTCHA responses, payment data, and government ID numbers.
...
3. Summarize unresolved facts, material edits, fees, travel or attendance obligations, legal terms, consents, and attachments.
...
- For a verified submission, create the immutable submitted Q&A record described in [references/tracker-and-archive.md](references/tracker-and-archive.md), link it from the tracker, and record organizer/recruiter domain, confirmation number, expected response timing, and next follow-up.
